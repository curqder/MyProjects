...
def add_data_for_name(name_data, year, rank, name):
    """
    :param name_data: dict, key is baby's name, value is another dict(key is year, value is rank)
    :param year: str, the year to be inputted
    :param rank: str, new baby's rank in the year to be inputted
    :param name: str, new baby's name to be inputted
    :return: None
    This function will input new data into the "name_data"(dict).
    """
    if name not in name_data:  # check first
        name_data[name] = {year: rank}
    else:  # name in name_date
        if year in name_data[name]:  # the same years
            if int(name_data[name][year]) > int(rank):
                name_data[name][year] = rank
        else:
            name_data[name][year] = rank  # not in the same year
