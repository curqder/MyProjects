...
def add_file(name_data, filename):
    ...
    with open(filename, 'r')as f:
        for line in f:
            ...
            if len(name_list) == 1:  # pick the year
...
def search_names(name_data, target):
    ...
    names = []
    # new_target = ''
    # for i in range(len(target)):  # case-insensitive
    #     if i == 0:
    #         new_target += target[i].upper()
    #     else:
    #         target = target.lower()
    #         new_target += target[i]

    for name in name_data:  # search name
        # target = target.lower()
        # if target in name:
        #     names.append(name)
        # if new_target in name:
        #     names.append(name)
