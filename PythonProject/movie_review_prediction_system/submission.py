...
def learnPredictor(trainExamples: List[Tuple[Any, int]], validationExamples: List[Tuple[Any, int]],
    ...
    def predictor(x):
        """
        @param x: str, 每個example的
        @return: int, 預測值 1 或 -1
        """
        y = 1 if dotProduct(featureExtractor(x), weights) >= 0 else -1
        return y

    for i in range(numEpochs):
        for line, label in trainExamples:
            k = dotProduct(weights, featureExtractor(line))
            h = sigmoid(k)
            if label == -1:
                label = 0
            scale = -(alpha * (h - label))
            increment(weights, scale, featureExtractor(line))
        # print('Training Error: ('+str(i)+' epoch): ', str(evaluatePredictor(trainExamples, predictor)))
        # print('Validation Error: ('+str(i)+' epoch): ', str(evaluatePredictor(validationExamples, predictor)))
    # END_YOUR_CODE
    return weights
...
def generateDataset(numExamples: int, weights: WeightVector) -> List[Example]:
    ...
    def generateExample() -> Tuple[Dict[str, int], int]:
        ...
        """
        # BEGIN_YOUR_CODE (our solution is 4 lines of code, but don't worry if you deviate from this)
        phi = defaultdict(zero)
        phi_len = random.randint(1, len(weights))
        word_list = []
        for key in weights:
            word_list.append(key)
        while len(phi) != phi_len:
            phi[word_list[random.randint(0, len(weights)-1)]] += 1
...
def extractCharacterFeatures(n: int) -> Callable[[str], FeatureVector]:
    ...
    def extract(x: str) -> Dict[str, int]:
        ...
        for i in range(len(lst)):
            new_x += lst[i]
        feature_vector = defaultdict(zero)
        for i in range(len(new_x)-n+1):
            feature_vector[new_x[i:i+n]] += 1
        return feature_vector
        # END_YOUR_CODE
